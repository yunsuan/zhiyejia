...
- (void)setModel:(HNChannelModel *)model {
    ...
    if (model.isRegular) {
        [self setTitleColor:COLOR(68, 68, 68, 1) forState:UIControlStateNormal];
    }
...
- (void)btnLong:(UILongPressGestureRecognizer *)ges {
    if (!self.model.isMyChannel || self.deleImageView.hidden) {
        return;
    }
...
//            self.frame.size.height = self.frame.size.height+ 8
            self.bounds = CGRectMake(0, 0, self.bounds.size.width+10, self.bounds.size.height+8);
            self.center = center;
        }];
        if (_beginBlock) {
            _beginBlock(self);
