...
- (XGToolBar *)xgToolBar{
    ...
    if (!_xgToolBar) {
        ...
        _xgToolBar = [[XGToolBar alloc] initWithFrame:CGRectMake(0, SCREEN_Height - 100 *SIZE, SCREEN_Width, 100 *SIZE) projectId:self.infoid albumArr:self.albumArr];
...
//        image = [UIImage getWaterMarkImage:image andTitle:@"云算云渠道" andMarkFont:nil andMarkColor:nil];
        image = [UIImage getAgentWaterImage:image andTitle:@"置业家" agent:model.name andMarkFont:nil andMarkColor:nil];
        UIImageWriteToSavedPhotosAlbum(image, self.class, @selector(completedWithImage:error:context:), (__bridge void *)self);
    }else{
        
        image = [UIImage getWaterMarkImage:image andTitle:@"置业家" andMarkFont:nil andMarkColor:nil];
        UIImageWriteToSavedPhotosAlbum(image, self.class, @selector(completedWithImage:error:context:), (__bridge void *)self);
    }
}
