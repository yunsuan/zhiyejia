...
@interface RentStoreDemandVC ()<UIScrollViewDelegate,UITextViewDelegate,UITextFieldDelegate,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
{
...
@property (nonatomic, strong) GZQFlowLayout *houseLayout;
...
@implementation RentStoreDemandVC
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (collectionView == _typeColl) {
        ...
        cell.typeImg.image = [cell grayscaleImageForImage:IMAGE_WITH_NAME(@"company_1")];
        cell.titleL.text = @"公司";
        return cell;
    }
}
...
- (void)initUI{
    ...
    _titleHeader.titleL.text = @"租房-商铺";
    ...
    _houseHeader.titleL.text = @"配套要求";
    _houseHeader.lineView.hidden = YES;
    [_contentView addSubview:_houseHeader];
    
    _houseLayout = [[GZQFlowLayout alloc] initWithType:AlignWithLeft betweenOfCell:SIZE];
    _houseLayout.itemSize = CGSizeMake(72 *SIZE, 60 *SIZE);
//    _houseLayout.minimumLineSpacing = SIZE;
//    _houseLayout.minimumInteritemSpacing = 0;
//    _houseLayout.scrollDirection = UICollectionViewScrollDirectionHorizontal;

    _houseColl = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_Width, 100 *SIZE) collectionViewLayout:_houseLayout];
    _houseColl.backgroundColor = CLWhiteColor;
    _houseColl.delegate = self;
    _houseColl.dataSource = self;
    [_houseColl registerClass:[EquipMentCollCell class] forCellWithReuseIdentifier:@"EquipMentCollCell"];
...
- (void)MasonryUI{
    ...
    [_areaSlider mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.mas_equalTo(340 *SIZE);
        make.height.mas_equalTo(40 *SIZE);
//        make.bottom.equalTo(self->_contentView.mas_bottom).offset(-26 *SIZE);
    }];
