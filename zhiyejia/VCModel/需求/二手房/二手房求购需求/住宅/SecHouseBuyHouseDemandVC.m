...
@interface SecHouseBuyHouseDemandVC ()<UIScrollViewDelegate,UITextViewDelegate,UITextFieldDelegate,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
{
...
@implementation SecHouseBuyHouseDemandVC
...
- (void)ActionHouseTypeBtn:(UIButton *)btn{
    ...
    [BaseRequest POST:NeedBuyAdd_URL parameters:dic success:^(id  _Nonnull resposeObject) {
        ...
        if ([resposeObject[@"code"] integerValue] == 200) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                if (self.secHouseBuyHouseDemandVCBlock) {
                    
                    self.secHouseBuyHouseDemandVCBlock();
                }
...
//    _priceHeader.lineView.hidden = YES;
    ...
    _houseTypeL.text = @"意向户型";
    [_contentView addSubview:_houseTypeL];

    _houseTypeBtn = [[DropDownBtn alloc] initWithFrame:CGRectMake(80 *SIZE, 0, 270 *SIZE, 33 *SIZE)];
    [_houseTypeBtn addTarget:self action:@selector(ActionHouseTypeBtn:) forControlEvents:UIControlEventTouchUpInside];
    ...
    _decorateL.font = [UIFont systemFontOfSize:13 *SIZE];
    _decorateL.text = @"装修要求";
    [_contentView addSubview:_decorateL];

    _decorateBtn = [[DropDownBtn alloc] initWithFrame:CGRectMake(80 *SIZE, 0, 270 *SIZE, 33 *SIZE)];
    [_decorateBtn addTarget:self action:@selector(ActionDecorateBtn:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)MasonryUI{
    ...
    [_buyUseL mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self->_contentView).offset(10 *SIZE);
        make.top.equalTo(self->_houseTypeBtn.mas_bottom).offset(15 *SIZE);
        make.width.mas_equalTo(70 *SIZE);
    }];
    ...
    [_buyUseBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self->_contentView).offset(80 *SIZE);
        make.top.equalTo(self->_houseTypeBtn.mas_bottom).offset(10 *SIZE);
        make.width.mas_equalTo(270 *SIZE);
        make.height.mas_equalTo(33 *SIZE);
    }];
